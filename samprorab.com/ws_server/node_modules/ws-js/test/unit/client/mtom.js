...
var MtomHandler = require('../../../lib/handlers/client/mtom/mtom.js').MtomClientHandler;
...
module.exports = {
	...
	"mime writer correctly writes attachments": function (test) {        				
		...
		//fs.writeFileSync("./test/unit/client/files/expected_writer_output.bin", body);
	...
	"mime reader correctly reads attachments and headers": function (test) {        				
		...
		var parts = reader.parse_multipart(buff, "my_unique_boundary");
		...
		var envelope_res = select(doc, "/x/file");
	...
	"add attachments works well": function(test) {
		...
		var file2 = fs.readFileSync(file2path);
		...
		test.equals(ctx.base64Elements.length, 2, "wrong number of base64 elements registered");
		test.equals(ctx.base64Elements[0].xpath, "/x/file1", "first registered base64 is wrong");
		test.equals(ctx.base64Elements[1].xpath, "/x/file2", "second registered base64 is wrong");
	...
	"handler correctly sets content type, xml body, and parts": function(test) {
		...
		var ctx = {
					request: "<x><str>abc</str><file1>AAAA</file1><file2>BBBB</file2></x>", 
    ...
    "handler correctly reads attachments from response": function(test) {
		 ...
		 var ctx = {
			 resp_contentType: 'multipart/related; type="application/xop+xml";start="<part0>";boundary="my_unique_boundary";start-info="old/contentType"; action="myAction"',
	...
	"ws.getAttachment should extract attachments": function(test) {
		...
		var buff = ws.getAttachment(ctx, "response", "/x/f");
