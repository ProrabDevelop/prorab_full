...
class view{
    ...
    public static function block($name, $module = null){

        $root_block = TEMPLATES.'blocks/'.$name.'.php';
        $module_block = APP.$module.'/templates/blocks/'.$name.'.php';

        if(file_exists($module_block)){
            require($module_block);
        }else{
            if(file_exists($root_block)){
                require($root_block);
            }else{

                if($module){
                    debugger::error('Блок: '.$name.' в модуле: '.$module.' не существует');
                }else{
                    debugger::error('Блок: '.$name.' не существует');
                }
