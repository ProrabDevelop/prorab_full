...
$('.set_edit_work_item').click(function (){
...
$('.lk_menu a').each(function(){
    ...
    if ( window.location.href.indexOf(href) >= 0 ){
        ...
        if($(this).hasClass('child')){
            $(this).parent().parent().parent().find('.has_child').addClass('active');
...
$('.review_complain').click(function (){
    var review_id = $(this).attr('review_id');
    var data = {};
            ...
            ajax_send_base(send_data, 'users/workwith', function (data){
                if(data.status === 'ok'){
                    ...
                    if($('.history_tbody tr').length === 0){
                        $('.history_wrapper').html('<h2>Список заказов пуст</h2>');
                    }
...
$(document).on('submit', '.add_review_answer', function (e){
    ...
    ajax_send_base(data, 'reviews/add_answer', function (data){
        ...
        if(data.status === 'ok'){
            ...
            $(reviews_item).find('.review_controls').prepend('<p class="view_all_review_cont_in_lk"><i class="icon icon-review_all"></i>Посмотреть ответ</p>');
            $(reviews_item).find('.review_answer').remove();
...

{
    ...
    $(document).on('click', '.work_with_master', function (){
        ...
        ajax_send_base(data, 'users/workwith', function (data){
            if(data.status === 'ok'){
                ...
                if(data.data.worked === true){
                    ...
                    $('.work_with_master').text('Работаю с мастером');
                }
...

{
    ...
    $(function (){
        ...
        if($('.lk_menu .has_child').hasClass('active')){

            var open_menu = true;

            var sub_menu_items = $('.lk_menu .has_child').next().find('li');

            $(sub_menu_items).each(function (key, link){
